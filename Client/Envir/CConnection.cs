...
namespace Client.Envir
{
    public sealed class CConnection : BaseConnection
    {
        ...
        public void Process(S.NPCCraft p)
        {
            if (p.Ingredients == null) return;

            foreach (CellLinkInfo cellLinkInfo in p.Ingredients)
            {
                DXItemCell[] grid;
            ...
            if (p.Success)
                GameScene.Game.NPCCraftBox?.UpdateRecipeDisplay();
        }
