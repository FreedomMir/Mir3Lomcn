...
namespace Client.Scenes.Views
{
    public sealed partial class NPCDialog : DXWindow
    {
        ...
        public override void OnIsVisibleChanged(bool oValue, bool nValue)
        {
            ...
            if (GameScene.Game.NPCCraftBox != null && !IsVisible)
        ...
        public void Response(S.NPCResponse info)
        {
            ...
            switch (info.Page.DialogType)
            {
                ...
                case NPCDialogType.ItemFragment:
                    ...
                    break;
                case NPCDialogType.Craft:
                    GameScene.Game.NPCCraftBox.Visible = true;
                    GameScene.Game.NPCCraftBox.Location = new Point(Size.Width - GameScene.Game.NPCCraftBox.Size.Width, Size.Height);
                    GameScene.Game.NPCCraftBox.LoadRecipes(Page);
                    break;
    ...
    public sealed class NPCCraftDialog : DXWindow
    {
        public DXComboBox RecipeComboBox;
        public DXItemGrid IngredientGrid;
        public DXButton CraftButton;
        public DXLabel GoldLabel, ChanceLabel, IngredientsLabel, ResultLabel;

        private readonly List<CraftRecipeInfo> Recipes = new List<CraftRecipeInfo>();
        private CraftRecipeInfo SelectedRecipe;
        ...
        public override void OnIsVisibleChanged(bool oValue, bool nValue)
        {
            ...
            if (!IsVisible)
            {
                IngredientGrid.ClearLinks();
                Recipes.Clear();
                SelectedRecipe = null;

                foreach (DXControl control in RecipeComboBox.ListBox.Controls.ToArray())
                {
        ...
        public NPCCraftDialog()
        {
            ...
            RecipeComboBox = new DXComboBox
            {
                Parent = this,
                Size = new Size(280, DXComboBox.DefaultNormalHeight),
                Location = new Point(label.Location.X, label.Location.Y + label.Size.Height + 5),
            };
            RecipeComboBox.SelectedItemChanged += (o, e) =>
            {
                SelectedRecipe = RecipeComboBox.SelectedItem as CraftRecipeInfo;
                UpdateRecipeDisplay();
            };

            ResultLabel = new DXLabel
            {
                Parent = this,
                AutoSize = false,
                Size = new Size(280, 40),
                Location = new Point(label.Location.X, RecipeComboBox.Location.Y + RecipeComboBox.Size.Height + 5),
                ForeColour = Color.White,
                DrawFormat = TextFormatFlags.WordBreak,
            };

            IngredientsLabel = new DXLabel
            {
                Text = "Materials",
                Parent = this,
                Location = new Point(label.Location.X, ResultLabel.Location.Y + ResultLabel.Size.Height + 5),
                Font = new Font(Config.FontName, CEnvir.FontSize(8F), FontStyle.Underline)
            };

            IngredientGrid = new DXItemGrid
            {
                GridSize = new Size(6, 1),
                Parent = this,
                GridType = GridType.CraftIngredient,
                Linked = true,
                Location = new Point(IngredientsLabel.Location.X + 5, IngredientsLabel.Location.Y + IngredientsLabel.Size.Height + 5)
            };

            foreach (DXItemCell cell in IngredientGrid.Grid)
                cell.LinkChanged += (o, e) => UpdateCraftButton();
            ...
            GoldLabel = new DXLabel
            {
                ...
                AutoSize = false,
                Size = new Size(280, 20),
                Location = new Point(label.Location.X, IngredientGrid.Location.Y + IngredientGrid.Size.Height + 10),
                ForeColour = Color.White,
            };
            ...
            ChanceLabel = new DXLabel
            {
                ...
                AutoSize = false,
                Size = new Size(280, 20),
                Location = new Point(label.Location.X, GoldLabel.Location.Y + GoldLabel.Size.Height + 5),
                ForeColour = Color.White,
            };
            ...
            {
                Parent = this,
                Location = new Point(label.Location.X, ChanceLabel.Location.Y + ChanceLabel.Size.Height + 10),
                Size = new Size(120, SmallButtonHeight),
                ButtonType = ButtonType.SmallButton,
                Label = { Text = "Craft" },
                Enabled = false,
            };
            CraftButton.MouseClick += (o, e) =>
            {
                if (GameScene.Game.Observer || SelectedRecipe == null) return;
                ...
                CEnvir.Enqueue(new C.NPCCraft { RecipeIndex = SelectedRecipe.Index, Ingredients = links });
            };

            SetClientSize(new Size(300, CraftButton.Location.Y + CraftButton.Size.Height + 10 - ClientArea.Y));
        }

        public void LoadRecipes(NPCPage page)
        {
            Recipes.Clear();
            SelectedRecipe = null;
            ...
            if (Globals.CraftRecipeInfoList?.Binding == null)
            {
                UpdateRecipeDisplay();
                return;
            }

            foreach (CraftRecipeInfo recipe in Globals.CraftRecipeInfoList.Binding)
            {
                if (!recipe.Enabled || recipe.ResultItem == null || recipe.Ingredients.Count == 0) continue;
                if (!RecipeMatchesCategory(recipe, page)) continue;

                Recipes.Add(recipe);

                new DXListBoxItem
                {
                    Parent = RecipeComboBox.ListBox,
            ...
            if (Recipes.Count > 0)
                RecipeComboBox.ListBox.SelectItem(Recipes[0]);
            else
            {
                SelectedRecipe = null;
                UpdateRecipeDisplay();
        ...
        private static bool RecipeMatchesCategory(CraftRecipeInfo recipe, NPCPage page)
        {
            if (string.IsNullOrWhiteSpace(page.Arguments)) return true;

            string[] categories = page.Arguments.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

            if (categories.Length == 0) return true;

            if (string.IsNullOrWhiteSpace(recipe.Category)) return false;

            return categories.Any(category => string.Equals(category, recipe.Category, StringComparison.OrdinalIgnoreCase));
        }
        ...
        public void UpdateRecipeDisplay()
        {
            ...
            {
                ResultLabel.Text = "No recipes available.";
                IngredientsLabel.Text = "Materials";
            ...
            ResultLabel.Text = $"Creates: {SelectedRecipe.ResultItem.ItemName} x{SelectedRecipe.ResultAmount}";
            IngredientsLabel.Text = "Materials";

            List<string> ingredientText = new List<string>();
            foreach (CraftRecipeIngredientInfo ingredient in SelectedRecipe.Ingredients)
            {
                if (ingredient.Item == null) continue;

                ingredientText.Add($"{ingredient.Item.ItemName} x{ingredient.Amount}");
            }

            GoldLabel.Text = $"Gold Cost: {SelectedRecipe.GoldCost:#,##0}";
            GoldLabel.ForeColour = SelectedRecipe.GoldCost > MapObject.User.Gold.Amount ? Color.Red : Color.White;
            ChanceLabel.Text = $"Success Chance: {SelectedRecipe.SuccessChance}%";

            if (ingredientText.Count > 0)
                ResultLabel.Text += Environment.NewLine + string.Join(", ", ingredientText);
        ...
        private void UpdateCraftButton()
        {
            ...
            if (SelectedRecipe.GoldCost > MapObject.User.Gold.Amount || MapObject.User.Level < SelectedRecipe.RequiredLevel)
            {
                CraftButton.Enabled = false;
                return;
            }

            if (SelectedRecipe.RequiredClass != RequiredClass.None && !MatchesRequiredClass(MapObject.User.Class, SelectedRecipe.RequiredClass))
            {
                CraftButton.Enabled = false;
                return;
            }
            ...
            foreach (CraftRecipeIngredientInfo ingredient in SelectedRecipe.Ingredients)
            {
                ...
                if (!required.ContainsKey(ingredient.Item.Index))
                    required[ingredient.Item.Index] = 0;

                required[ingredient.Item.Index] += ingredient.Amount;
            }

            Dictionary<int, int> provided = new Dictionary<int, int>();
            foreach (DXItemCell cell in IngredientGrid.Grid)
            {
                if (cell.Link?.Item == null) continue;

                if (!required.ContainsKey(cell.Link.Item.Info.Index))
                {
                    CraftButton.Enabled = false;
                    return;
                }

                if (!provided.ContainsKey(cell.Link.Item.Info.Index))
                    provided[cell.Link.Item.Info.Index] = 0;

                provided[cell.Link.Item.Info.Index] += (int)cell.LinkedCount;
            }

            foreach (KeyValuePair<int, int> pair in required)
            {
                if (!provided.TryGetValue(pair.Key, out int amount) || amount != pair.Value)
                {
                    CraftButton.Enabled = false;
                    return;
                }
        ...
        private static bool MatchesRequiredClass(MirClass playerClass, RequiredClass required)
        {
            switch (playerClass)
            {
                case MirClass.Warrior:
                    return (required & RequiredClass.Warrior) == RequiredClass.Warrior;
                case MirClass.Wizard:
                    return (required & RequiredClass.Wizard) == RequiredClass.Wizard;
                case MirClass.Taoist:
                    return (required & RequiredClass.Taoist) == RequiredClass.Taoist;
                case MirClass.Assassin:
                    return (required & RequiredClass.Assassin) == RequiredClass.Assassin;
                default:
                    return false;
            }
        }
