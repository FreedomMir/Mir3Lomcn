...
namespace Server.Models
{
    public partial class PlayerObject : MapObject
    {
        ...
        public void NPCCraft(C.NPCCraft p)
        {
            S.NPCCraft result = new S.NPCCraft
            {
            ...
            if (Dead || NPC == null || NPCPage == null || NPCPage.DialogType != NPCDialogType.Craft) return;

            CraftRecipeInfo recipe = SEnvir.CraftRecipeInfoList.Binding.FirstOrDefault(x => x.Index == p.RecipeIndex);

            if (recipe == null || !recipe.Enabled || recipe.ResultItem == null || recipe.Ingredients.Count == 0 || !RecipeMatchesCategory(recipe, NPCPage))
            {
                Connection.ReceiveChatWithObservers(con => con.Language.NPCCraftRecipeInvalid, MessageType.System);
                return;
            }

            if (Level < recipe.RequiredLevel)
            {
                Connection.ReceiveChatWithObservers(con => string.Format(con.Language.NPCCraftLevel, recipe.RequiredLevel), MessageType.System);
                return;
            }

            if (recipe.RequiredClass != RequiredClass.None && !MatchesRequiredClass(Class, recipe.RequiredClass))
            {
                Connection.ReceiveChatWithObservers(con => con.Language.NPCCraftClass, MessageType.System);
                return;
            }

            if (!ParseLinks(p.Ingredients, 0, 6)) return;

            Dictionary<int, int> required = new Dictionary<int, int>();

            foreach (CraftRecipeIngredientInfo ingredient in recipe.Ingredients)
            {
            ...
            foreach (CellLinkInfo link in p.Ingredients)
            {
                UserItem[] array;
                switch (link.GridType)
                {
                ...
                if (!required.ContainsKey(item.Info.Index))
                {
                    Connection.ReceiveChatWithObservers(con => con.Language.NPCCraftIngredients, MessageType.System);
                    return;
                }

                if (!provided.ContainsKey(item.Info.Index))
                    provided[item.Info.Index] = 0;

                provided[item.Info.Index] += (int)link.Count;
            }
            ...
            if (recipe.GoldCost > Gold.Amount)
            {
                Connection.ReceiveChatWithObservers(con => string.Format(con.Language.NPCCraftGold, recipe.GoldCost - Gold.Amount), MessageType.System);
                return;
            }

            bool craftSuccess = SEnvir.Random.Next(100) < recipe.SuccessChance;

            if (craftSuccess)
            {
                ItemCheck check = new ItemCheck(recipe.ResultItem, recipe.ResultAmount, UserItemFlags.None, TimeSpan.Zero);

                if (!CanGainItems(false, check))
                {
                    Connection.ReceiveChatWithObservers(con => con.Language.NPCCraftNoRoom, MessageType.System);
                    return;
                }
            }
            ...
            if (!craftSuccess)
            {
                ...
                Connection.ReceiveChatWithObservers(con => con.Language.NPCCraftFailed, MessageType.System);
                return;
            }

            ItemCheck gainCheck = new ItemCheck(recipe.ResultItem, recipe.ResultAmount, UserItemFlags.None, TimeSpan.Zero);
            while (gainCheck.Count > 0)
                GainItem(SEnvir.CreateFreshItem(gainCheck));

            result.Crafted = true;
            Connection.ReceiveChatWithObservers(con => string.Format(con.Language.NPCCraftSuccess, recipe.ResultItem.ItemName, recipe.ResultAmount), MessageType.System);
        }
